...
class ExpoPushService:
    ...
    async def send_push_notification(self, token: str, title: str, body: str, data: dict = None):
        ...
        For data-only (silent) types — 'ping' and 'location_ping' — no title/body/sound
        is sent so the notification never surfaces in the device shade or lock screen.
        The device OS still delivers the data payload and wakes the app's background
        task handler (registerTaskAsync / content-available=1) to transmit a GPS fix.
        """
        import httpx

        notif_type = (data or {}).get("type", "")

        # ── Silent / data-only types ───────────────────────────────────────────
        # 'ping'          → security officer tracking a civil user
        # 'location_ping' → admin/security requesting security agent location refresh
        # Neither should be visible to the recipient — they must NOT appear in
        # the notification shade, lock screen, or sound an alert.
        SILENT_TYPES = {"ping", "location_ping"}
        is_silent = notif_type in SILENT_TYPES

        # Map notification type → Android channel (must match channels created in notifications.ts)
        #   emergency → panic alerts   (IMPORTANCE_MAX, bypassDnd, heavy vibration)
        #   report    → video/audio    (IMPORTANCE_HIGH, default sound)
        #   default   → chat messages  (IMPORTANCE_HIGH, default sound)
        channel_map = {
            "panic":               "emergency",
            "emergency":           "emergency",
            "report":              "report",
            ...
            "message":             "default",
            "escort_started":      "default",   # security agents notified of new escort
            "escort_eta_check":    "default",   # civil user welfare check at ETA
            "escort_eta_elapsed":  "default",   # security agents notified of ETA elapsed
        }
        channel_id = channel_map.get(notif_type, "default")
        is_emergency = channel_id == "emergency"

        if is_silent:
            # Silent / data-only push — no visible notification whatsoever.
            # 'content-available': 1  → iOS wakes the background app extension
            # '_contentAvailable': 1  → Expo/FCM equivalent for Android background wakeup
            # No title, body, sound, or channelId so Android shows nothing.
            # Priority 'high' still required to wake the device out of Doze.
            message = {
                "to":                token,
                "data":              data or {},
                "priority":          "high",
                "content-available": 1,
                "_contentAvailable": 1,
                "ttl":               60,   # 1 min — if undelivered, drop it
            }
            logger.info(f"[Push] Sending SILENT type={notif_type} token={token[:20]}…")
        else:
            message = {
                "to":    token,
                "title": title,
                ...
                "data":  data or {},
                # ── Sound ──────────────────────────────────────────────────────
                # "default" uses the device's default notification sound — the same
                # sound produced by WhatsApp, Signal, etc.  Without this the Expo
                # push service sends a silent notification on many Android OEMs.
                "sound":    "default",
                # ── Android priority ───────────────────────────────────────────
                # "high" ensures FCM delivers immediately even in Doze mode and
                # triggers a heads-up notification (peek) from the notification shade.
                "priority": "high",
                # ── Android channel ────────────────────────────────────────────
                # Must match a channel created by setNotificationChannelAsync() in
                # notifications.ts.  The channel controls importance, sound, and
                # vibration pattern on Android 8+.
                "channelId": channel_id,
                # ── iOS: allow background content update ───────────────────────
                "content-available": 1,
                # ── Delivery TTL ───────────────────────────────────────────────
                # 0 = deliver now or drop; avoids stale panic/chat alerts.
                "ttl": 0,
            }

            if is_emergency:
                message["vibrate"] = [0, 500, 200, 500, 200, 500]
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                status = result.get("data", [{}])[0].get("status", "unknown") if "data" in result else "ok"
                logger.info(f"[Push] Sent type={notif_type} channel={channel_id} status={status} token={token[:20]}…")
                return result
        except Exception as e:
            logger.error(f"[Push] send_push_notification error: {e}")
            return None
