...
import { View, Text, TouchableOpacity, StyleSheet, FlatList, ActivityIndicator, TextInput, Modal, RefreshControl, KeyboardAvoidingView, Platform, Image } from 'react-native';
...
export default function CivilMessages() {
  ...
  const openConversation = async (conv: any) => {
    ...
    // Mark this conversation as read on the backend so the badge clears
    try {
      const token = await getAuthToken();
      if (token) {
        ...
        // Refresh conversations list so unread count badge updates immediately
        loadData();
      }
    } catch (_) {}
  };
  ...
  const renderConversation = ({ item }: any) => {
    const avatarColor = item.other_user?.role === 'security' ? '#F59E0B20' : '#3B82F620';
    const iconColor = item.other_user?.role === 'security' ? '#F59E0B' : '#3B82F6';
    return (
      ...
      <View style={[styles.avatarCircle, { backgroundColor: avatarColor }]}>
        {item.other_user?.profile_photo_url ? (
          <Image
            ...
            style={[styles.avatarCircle, { width: 50, height: 50, borderRadius: 25 }]}
          />
        ) : (
